...
let ngaykhoihanh:string = localStorage.getItem('NgayKhoiHanh');
let day:string =  ngaykhoihanh.slice(8,10) +"/" + ngaykhoihanh.slice(5,7) +"/"+ ngaykhoihanh.slice(0,4)  ;
LoadTour( danhSachTour.MangTour,ngaykhoihanh) ;
$('.ngaykhoihanh').html(`<h2 class="text-center"> Departure Day:  <span class="ml-3">${day}</span></h2>`)



function LoadTour(data:any,ngaykhoihanh){
   let content:string = "";
   let danhSachTour = new DanhSachTour();

   data.map(function(tour:Tour){
      
      if(tour.NgayKhoiHanh.slice(0,10) === ngaykhoihanh){
